...
public class TicketsMenu extends DiscordSelectMenu {
  ...
  @Override
  public void onSelect(StringSelectInteraction interaction,
      List<DiscordSelectMenuOption> selectedOptions) {
    ...
    if (ticketType.equals(
        TicketType.REPORT)) { // TODO: 18.08.2024 10:32 - special treatment for ChannelCreationModals
  ...
  private void handleWhitelist(TicketType ticketType, StringSelectInteraction interaction) {
    ...
    WhitelistDTO.isWhitelisted(user).thenAcceptAsync(whitelistedBoolean -> {
      ...
      interaction.editSelectMenu(interaction.getSelectMenu()).queue();
      DataApi.getDataInstance()
          .logError(getClass(), "Error while checking if user is whitelisted", failure);
  ...
  private void handleUnban(StringSelectInteraction interaction) {
    ...
    reportTicketModal.startChannelCreation(interaction)
        .exceptionally(failure -> {
          DataApi.getDataInstance()
              .logError(getClass(), "Error while creating unban ticket", failure);
          return null;
        });
  ...
  private void handleReport(TicketType ticketType, StringSelectInteraction interaction) {
    ...
    reportTicketModal.startChannelCreation(interaction)
        .exceptionally(failure -> {
          DataApi.getDataInstance()
              .logError(getClass(), "Error while creating report ticket", failure);
          return null;
        });
  }
