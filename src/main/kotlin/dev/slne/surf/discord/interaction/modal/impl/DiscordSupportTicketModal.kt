...
class DiscordSupportTicketModal(
    ...
    override fun create() = modal(id, "Discord Support Ticket") {
        field {
            id = "issue"
    ...
    override suspend fun onSubmit(event: ModalInteractionEvent) {
        ...
        val ticket =
            ticketService.createTicket(
                interaction.hook,
                TicketType.SUPPORT_DISCORD,
                TicketData.of("issue" to issue)
            ) ?: run {
                if (ticketService.hasOpenTicket(user.idLong, TicketType.SUPPORT_DISCORD)) {
                    interaction.hook.editOriginal("Du hast bereits ein offenes Discord Support Ticket.")
                        .queue()
                } else {
        ...
        thread.sendMessageEmbeds(
            embed {
                ...
                description =
                    "Dein Discord Support Ticket wurde erstellt und wir haben deine Informationen erhalten. Bitte habe ein wenig Geduld, während das Team dein Anliegen bearbeitet."
