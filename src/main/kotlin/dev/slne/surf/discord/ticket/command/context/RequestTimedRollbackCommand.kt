...
@DiscordCommand(
    name = "request-timed-rollback",
    description = "Beantragt einen zeitlich begrenzten Rollback für einen Spieler.",
    options = [CommandOption(
        ...
        name = "grund",
        description = "Der Grund für den Rollback",
        type = CommandOptionType.STRING,
        required = true
    ), CommandOption(
        name = "zeit",
...
object RequestTimedRollbackCommand : SlashCommand {
    override suspend fun execute(event: SlashCommandInteractionEvent) {
        ...
        event.replyEmbeds(embed {
            ...
            title = translatable("rollback.command.timed.title", minecraftName)
            ...
            field {
                name = translatable("rollback.command.timed.field.time")
                value = time
                inline = false
            }
            world?.let {
