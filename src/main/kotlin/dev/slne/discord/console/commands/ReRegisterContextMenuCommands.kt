...
class ReRegisterContextMenuCommands(
    private val jda: JDA,
    private val commandProcessor: DiscordContextMenuCommandProcessor
) : SuspendingCliktCommand("context") {
    ...
    override fun help(context: Context) = buildString {
        currentContext.theme.apply {
            appendLine(info("Reregister context menu commands for a guild or all guilds."))
    ...
    override suspend fun run() {
        echo(currentContext.theme.info("Reregistering context menu commands..."))
        val guildId = guildId
    ...
    private suspend fun updateCommands(guild: Guild) {
        echo(currentContext.theme.info("Reregistering context menu commands for guild: ${guild.name}"))
